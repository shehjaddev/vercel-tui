...
func (c *Client) EnvVars(ctx context.Context, projectID, teamID string) ([]EnvVar, error) {
	var out struct {
		Envs []EnvVar `json:"envs"`
	}
	if err := c.get(ctx, "/v10/projects/"+projectID+"/env", scoped(url.Values{}, teamID), &out); err != nil {
		return nil, err
	}
	return out.Envs, nil
}
...
func (c *Client) CreateEnv(ctx context.Context, projectID, teamID, key, value string, targets []string) error {
	body := map[string]any{
		"key":    key,
		"value":  value,
		"type":   "encrypted",
	...
	return c.request(ctx, "POST", "/v10/projects/"+projectID+"/env", scoped(url.Values{}, teamID), body, nil)
}

func (c *Client) UpdateEnvValue(ctx context.Context, projectID, teamID, envID, value string, targets []string) error {
	q := url.Values{"upsert": {"true"}}
	body := map[string]any{"value": value}
	if len(targets) > 0 {
		body["target"] = targets
	}
	return c.request(ctx, "PATCH", "/v10/projects/"+projectID+"/env/"+envID, scoped(q, teamID), body, nil)
}
